...
// ---- Repository file tree (real customer-data/ structure) -------------------
// Structural snapshot of the scanned repo (generated by scripts/build-file-tree.ts
// from customer-data/evenup/). Findings are joined onto it at runtime by path.
export interface FileTreeNode {
  name: string;
  path: string; // relative to customer-data/ (e.g. "evenup/docs/.../icd10_map.go")
  type: 'dir' | 'file';
  ext?: string;
  children?: FileTreeNode[];
}

// The same tree annotated with finding aggregates for rendering.
export interface AnnotatedTreeNode {
  ...
  findingCount: number;        // findings at/under this node
  highestPriority?: Priority;  // worst finding at/under this node (undefined ⇒ clean)
  findingIds: number[];        // Finding.id values located here (files only; [] for dirs)
}
