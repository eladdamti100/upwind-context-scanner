...
export const FINDINGS: Finding[] = RAW.map((raw, i) => {
  const { file, candidate, regex, local_context, scan_metadata, ground_truth: gt } = raw;
  ...
  // Build FindingContextObject (camelCase)
  const ctx = {
    ...
    scanMetadata: {
      sensitivityMode: (scan_metadata.sensitivity_mode || 'balanced') as Finding['activity'] extends never ? never : 'strict' | 'balanced' | 'flexible',
  ...
  return {
    ...
    scores: {
      ...
      lgbmProbability: lgbm.secretProbability,
    ...
    riskUpReasons: rules.triggered
      ...
      .map(t => t.label),
    riskDownReasons: rules.triggered
